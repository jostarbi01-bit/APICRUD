...
import { User, Mail, Lock, Loader2, AlertCircle, CheckCircle2, EyeOff, Eye } from "lucide-react";
...
export default function SignUpCard({ lang, onSuccess, onSwitchToSignIn }: SignUpProps) {
  ...
  // NEW: State tracking registration key character layouts
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await signUp.email({
      ...
      fetchOptions: {
        ...
        onError: (ctx) => {
          setIsPending(false);
          setErrorMessage(ctx.error.message || t.failedProvision);
        },
      },
    });
  };
  ...
  if (successState) {
    return (
      <div className="w-full max-w-md mx-auto p-8 bg-card text-card-foreground border rounded-xl shadow-sm text-center space-y-3">
        <div className="inline-flex p-3 bg-green-500/10 border border-green-500/20 rounded-full text-green-500 mx-auto">
          <CheckCircle2 className="h-6 w-6 animate-bounce" />
        </div>
        <h3 className="text-lg font-bold">{t.successProvision}</h3>
        <p className="text-xs text-muted-foreground">{t.syncingDb}</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full max-w-md mx-auto p-6 bg-card text-card-foreground border rounded-xl shadow-sm transition-all duration-200">
      <div className="text-center space-y-1 mb-6">
        <h2 className="text-2xl font-extrabold tracking-tight">{t.signUpTitle}</h2>
        <p className="text-xs text-muted-foreground">{t.signUpSubtitle}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-1.5">
          <label className="text-sm font-semibold text-muted-foreground">{t.nameLabel}</label>
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="operator@internal.local"
              className="w-full text-sm bg-background border rounded-md py-2 pl-10 pr-3 focus:ring-1 focus:ring-primary outline-none text-foreground"
              required
              disabled={isPending}
            />
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <input
              type={showPassword ? "text" : "password"} // Dynamic configuration switch layout type
              value={password}
        ...
        >
          {isPending ? (
            ...
            <span>{t.btnDeploySession}</span>
          )}
        </button>
      </form>

      <div className="mt-6 text-center text-sm">
        <span className="text-muted-foreground">{t.alreadyRegistered} </span>
        <button
